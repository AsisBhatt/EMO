...
jQuery(document).ready(function($){
	...
	var $Sms_sendAt = $('#SmsCampaign_send_at'), 
        $Sms_displaySendAt = $('#SmsCampaign_sendAt'),
        $Sms_fakeSendAt = $('#fake_sms_send_at');
	
    if ($Sms_sendAt.length && $Sms_displaySendAt.length && $Sms_fakeSendAt.length) {
        ...
        $Sms_displaySendAt.on('focus', function(){
            $('#fake_sms_send_at').datetimepicker('show');
        });
        
        function syncDateTime() {
            var date = $Sms_fakeSendAt.val();
            if (!date) {
                return;
            }
            $Sms_displaySendAt.val('').addClass('spinner');
            $.get($Sms_fakeSendAt.data('syncurl'), {date: date}, function(json){
                $Sms_displaySendAt.removeClass('spinner');
                $Sms_displaySendAt.val(json.utcDateTime);
                $Sms_sendAt.val(json.utcDateTime);
            }, 'json');
        }
        syncDateTime();
	}
