...
jQuery(document).ready(function($){
    ...
    var _koHandlersInit = function(){
        ...
        var glance = function(){
			...
			self.email_count		= ko.observable(0);
			self.mobile_count		= ko.observable(0);
            ...
            self.load = function(forced){
                ...
                $.getJSON($('#glance-box').data('source'), {}, function(json){
					...
					self.email_count(json.email_count);
					self.mobile_count(json.mobile_count);
                    self.loading(false);
                });
            };
	...
	if(data != 0 ||  data != 0){
		...
		//function(data) {
			...
			var resetCanvas = function(){
			  $('#searching_data').remove(); // this is my <canvas> element
			  $('#chart-container').append('<canvas id="results-graph"><canvas>');
			};
