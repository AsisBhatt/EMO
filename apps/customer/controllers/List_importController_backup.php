    ...
    public function actionCsv($list_uid)
    {
        ...
        try {
            ...
            try {
				...
				}else if($list->type == 'sms_list'){
					foreach ($data as $index => $fields) {

						$mobile = null;
						foreach ($fields as $detail) {
							// && !empty($detail['tagValue'])
							if ($detail['tagName'] == 'MOBILE') {
						...
						/*$importLog[] = array(
							'type'    => 'info',
							'message' => Yii::t('list_import', 'Checking the list for the mobile: "{mobile}"', array(
						...
						if (empty($subscriber)) {
							...
							$validEmail = !empty($mobile) && $validator->validateValue($mobile);*/

							/*if (!$validEmail) {
								$subscriber->addError('mobile', Yii::t('list_import', 'Invalid mobile address!'));
							} else {
								echo 'b';exit;
								$blacklisted = $subscriber->getIsBlacklisted();
								if (!empty($blacklisted)) {
