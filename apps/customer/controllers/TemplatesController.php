...
class TemplatesController extends Controller
{
    ...
    public function actionGallery_import($template_uid)
    {
        ...
        if (!($newTemplate = $template->copy())) {
			...
			CommonHelper::setActivityLogs('Email Template Gallery_import Error '.str_replace(array('{{','}}'),'',$template->tableName()),$template->template_id,$template->tableName(),'Email Template Gallery_import',(int)Yii::app()->customer->getId());
		...
		CommonHelper::setActivityLogs('Email Template Gallery_import '.str_replace(array('{{','}}'),'',$template->tableName()),$template->template_id,$template->tableName(),'Email Template',(int)Yii::app()->customer->getId());

        if (!$newTemplate->save(false)) {
            $newTemplate->delete();
			
			CommonHelper::setActivityLogs('Email Template Gallery_import Unable to imported '.str_replace(array('{{','}}'),'',$template->tableName()),$template->template_id,$template->tableName(),'Email Template Unable to imported',(int)Yii::app()->customer->getId());
    ...
    public function actionCopy($template_uid)
    {
        ...
        if (!($newTemplate = $template->copy())) {
			...
			CommonHelper::setActivityLogs('Email Template Copy Unable to copy '.str_replace(array('{{','}}'),'',$template->tableName()),$template->template_id,$template->tableName(),'Email Template Copy',(int)Yii::app()->customer->getId());
            
			Yii::app()->notify->addError(Yii::t('email_templates', 'Unable to copy the template!'));
            $this->redirect(array('templates/index'));
        }
		
		CommonHelper::setActivityLogs('Email Template Copy '.str_replace(array('{{','}}'),'',$template->tableName()),$template->template_id,$template->tableName(),'Email Template Copy',(int)Yii::app()->customer->getId());
    ...
    public function actionCreate()
    {
        ...
        if ($request->isPostRequest && ($attributes = $request->getPost($template->modelName, array()))) {
            ...
            if ($template->save()) {
				...
				CommonHelper::setActivityLogs('Email Template Create '.str_replace(array('{{','}}'),'',$template->tableName()),$template->template_id,$template->tableName(),'Email Template Create',(int)Yii::app()->customer->getId());
    ...
    public function actionUpdate($template_uid)
    {
        ...
        if ($request->isPostRequest && $attributes = $request->getPost($template->modelName, array())) {
			...
			CommonHelper::setActivityLogs('Email Template Update '.str_replace(array('{{','}}'),'',$template->tableName()),$template->template_id,$template->tableName(),'Email Template Update',(int)Yii::app()->customer->getId());
    ...
    public function actionSave_screenshot($template_uid)
    {
		...
		CommonHelper::setActivityLogs('Email Template Preview '.str_replace(array('{{','}}'),'',$template->tableName()),$template->template_id,$template->tableName(),'Email Template Preview',(int)Yii::app()->customer->getId());
    ...
    {
		ini_set("include_path", '/home/beelift/public_html/new2managevip/php:' . ini_get("include_path") );
        ...
        if ($request->isPostRequest && ($attributes = (array)$request->getPost($model->modelName, array()))) {
            ...
            if (!$model->validate() || !$model->uploader->handleUpload()){ 
				...
				CommonHelper::setActivityLogs('Email Template Upload Error '.str_replace(array('{{','}}'),'',$model->tableName()),$model->template_id,$model->tableName(),'Email Template Upload Error',(int)Yii::app()->customer->getId());
				
                Yii::app()->notify->addError($model->shortErrors->getAllAsString());
            } else {
				
				CommonHelper::setActivityLogs('Email Template Upload '.str_replace(array('{{','}}'),'',$model->tableName()),$model->template_id,$model->tableName(),'Email Template Upload',(int)Yii::app()->customer->getId());
    ...
    public function actionDelete($template_uid)
    {
		...
		CommonHelper::setActivityLogs('Email Template Delete '.str_replace(array('{{','}}'),'',$template->tableName()),$template->template_id,$template->tableName(),'Email Template Delete',(int)Yii::app()->customer->getId());
