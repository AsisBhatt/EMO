...
class List_importController extends Controller
{
    ...
    public function actionCsv($list_uid)
    {
        ...
        try {
            ...
            try {
				if($list->type == 'email_list'){
					
					foreach ($data as $index => $fields) {

						$email = null;
						foreach ($fields as $detail) {
							// && !empty($detail['tagValue'])
							if ($detail['tagName'] == 'EMAIL') {
								$email = $detail['tagValue'];
								break;
							}
						}
					...
					foreach ($data as $index => $fields) {
						...
						foreach ($fields as $detail) {
							// && !empty($detail['tagValue'])
							if ($detail['tagName'] == 'EMAIL' && $detail['tagValue'] != '') {
								$email = $detail['tagValue'];
								break;
							}else if($detail['tagName'] == 'MOBILE' && $detail['tagValue'] != ''){
								$mobile = $detail['tagValue'];
								break;
							}
						}
						...
						if (!empty($failures[$mobile])) {
							$importLog[] = array(
								'type'    => 'error',
								'message' => Yii::t('list_import', 'Failed to save the mobile "{mobile}", reason: {reason}', array(
									'{mobile}'  => CHtml::encode($mobile),
									'{reason}' => '<br />'.$failures[$mobile],
								)),
								'counter' => true,
							);
							continue;
						}else if (!empty($failures[$email])) {
							$importLog[] = array(
								'type'    => 'error',
								'message' => Yii::t('list_import', 'Failed to save the email "{email}", reason: {reason}', array(
									'{email}'  => CHtml::encode($email),
						...
						if ($subscriber == null) {
							/*if($mobile){
								$importLog[] = array(
									'type'    => 'info',
									'message' => Yii::t('list_import', 'The mobile "{mobile}" was not found, we will try to create it...', array(
										'{mobile}' => CHtml::encode($mobile),
									)),
									'counter' => false,
								);
							...
							if($email != ''){
								...
								if (!$validEmail) {
									...
									$blacklisted = $subscriber->getIsBlacklisted();
									if (!empty($blacklisted)) {
										$subscriber->addError('mobile', Yii::t('list_import', 'This mobile address is blacklisted!'));
									}
								}
							...
							if($fields[0]['tagName'] == 'EMAIL' && $fields[0]['tagValue'] != '' && $fields[3]['tagName'] == 'MOBILE' && $fields[3]['tagValue'] != '' && $validEmail){
								$subscriber->email   = ($fields[0]['tagValue'] != '' ? $fields[0]['tagValue'] : '');
								$subscriber->mobile   = ($fields[3]['tagValue'] != '' ? $fields[3]['tagValue'] : '');
								$subscriber->source  = ListSubscriber::SOURCE_IMPORT;
								$subscriber->status  = ListSubscriber::STATUS_CONFIRMED;
								$subscriber->save();
								...
								foreach($fields as $detail){								
									if($detail['tagName'] == 'MOBILE' && $detail['tagValue'] != '' || $detail['tagName'] == 'EMAIL' && $detail['tagValue'] != ''){
							...
							if($fields[3]['tagName'] == 'MOBILE' && $fields[3]['tagValue'] != ''){
								$importLog[] = array(
									'type'    => 'success',
									'message' => Yii::t('list_import', 'The mobile "{mobile}" has been successfully saved.', array(
										'{mobile}' => CHtml::encode($fields[3]['tagValue']),
									)),
									'counter' => true,
								);
							}else if(!empty($email)){
							...
							if(!empty($mobile)){
								$importLog[] = array(
									'type'    => 'info',
									'message' => Yii::t('list_import', 'The mobile "{mobile}" has been found, we will update it.', array(
										'{mobile}' => CHtml::encode($mobile),
									)),
									'counter' => true,
								);
								...
								$importLog[] = array(
									'type'    => 'info',
									'message' => Yii::t('list_import', 'The mobile "{email}" has been found, we will update it.', array(
										'{email}' => CHtml::encode($email),
									)),
									'counter' => true,
								);
						...
						foreach ($fields as $detail) {
								...
								if($detail['name'] == 'Mobile' && $detail['tagName'] == 'MOBILE'){
									$detail['tagValue'] = $this->clean($detail['tagValue']);
									$ext_array = array('ext','x','/');
									$detail['tagValue'] = str_replace($ext_array, ";", $detail['tagValue']);
									$detail['tagValue'] = explode(";",$detail['tagValue']);
									
									$detail['tagValue'] = (isset($detail['tagValue']) && $detail['tagValue'][0] != '' ? $detail['tagValue'][0] : '');
									
									/*;
									$invalid_characters = array("(", "+", ")", " ", "-", ".");
									$detail['tagValue'] = str_replace($invalid_characters, "", $detail['tagValue']);
									$detail['tagValue'] = (isset($detail['tagValue'][0]) && $detail['tagValue'][0] != '' ? $detail['tagValue'][0] : '');*/
								}
	...
	protected function clean($string) {
		...
		//$string = str_replace(array('x', 'ext','xt'), ';', $string);
