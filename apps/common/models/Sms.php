...
class Sms extends ActiveRecord
{
	...
	{
		$rules = array(
			array('mobile, type, message , response', 'required','on' => 'insert'),
			//array('mobile, type, message , media, response', 'required','on'=>'Sendmms'),

			// The following rule is used by search().
			array('customer_id, mobile, message, media, response,status, date_added', 'safe', 'on'=>'search'),
		);
	...
	public function sendSms($mobile, $message)
	{
		...
		if(isset($mobile) && isset($message)){
			...
			return $msg_status.'_'.$sms->sms_id;
		}
	}
	
	public function sendMms($mobile, $media_url, $message)
	{
		$customer = Yii::app()->customer->getModel();
		$sid = 3;
		$smssetting = Smssetting::model()->findByPk((int)$sid);
		$flowroute_sms_number = $customer->company->flowroute_sms_num;
		
		$data_array = array('to' => $mobile, 'from' => $flowroute_sms_number, 'body' => $message, 'is_mms' => "true", 'media_urls' => array($media_url));
		...
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
		curl_setopt($ch, CURLOPT_HTTPHEADER, array(
			'accesskey:'.trim($smssetting->access_key),
			'secretkey:'.trim($smssetting->authkey),
			'Content-Type: application/json',
			'Content-Length: ' . strlen($data_string))
		);
		
		$result = curl_exec($ch);
		$mms_obj = json_decode($result);
		
		$mms_id = '';
		$msg_status = '';
		if(isset($mms_obj->data) && $mms_obj->data != ''){
				$mms_id  = $mms_obj->data->id;
				$msg_status .= 'sent';
			}else if(isset($mms_obj->errors)){
				$mms_id = $mms_obj->errors[0]->id;
				$msg_status .= $mms_obj->errors[0]->detail;
			}
	...
	public function shceduleMms($mobile, $media_url, $message, $customer_id)
	{
		if(isset($mobile) && isset($message) && isset($media_url)){
			...
			$data_array = array('to' => $mobile, 'from' => $customer_company->flowroute_sms_num, 'body' => $message, 'is_mms' => "true", 'media_urls' => array($media_url));
	...
	public function sendAutoReply($mobile, $message, $customer_id)
	{
		if(isset($mobile) && isset($message)){
			...
			//$sender = '15613959554';
			$customer_company = CustomerCompany::model()->findByAttributes(array('customer_id'=> $customer_id));
			$sender = $customer_company->flowroute_sms_num;
			$data_array = array('to' => $mobile, 'from' => $sender, 'body' => $message);
