...
class ListSubscriber extends ActiveRecord
{
    ...
    public function rules()
    {
        $rules = array(
            ...
            array('list_id, subscriber_uid, email, mobile, source, ip_address, status', 'safe', 'on' => 'search'),
        );
    ...
    public function attributeLabels()
    {
        $labels = array(
			...
			'mobile'             => Yii::t('list_subscribers', 'Mobile'),
	...
	public function getStop($stop_number_array, $customer_id)
	{
		if(count($stop_number_array))
		{	
			$number_block = array();
			foreach($stop_number_array as $stop_number){
				$number_block[] = $stop_number['uic_list_field_value'];
			}
			
			$criteria = new CDbCriteria();
			$criteria->addInCondition('subscriber_id', $number_block);
			$subscribe = ListSubscriber::model()->findAll($criteria);
			
			$subscriber_id_array = array();
			foreach($subscribe as $subscribe_key => $subscribe_value){
				$subscriber_id_array[] = $subscribe_value['subscriber_id'];
			}
			
			$subscriber_id = implode(",",$subscriber_id_array);
			
			CommonHelper::setActivityLogs('Subscriberstop  '.str_replace(array('{{','}}'),'',$this->tableName()),$subscriber_id,$this->tableName(),'Subscriberstop',$customer_id);
	...
	public function getActive($active_number_array)
	{
		$customer  = Yii::app()->customer->getModel();
		
		if(count($active_number_array))
		{	
			$number_active = array();
			foreach($active_number_array as $active_number){
				$number_active[] = $active_number['uic_list_field_value'];
			}
			
			$criteria = new CDbCriteria();
			$criteria->addInCondition('subscriber_id', $number_active);
			$subscribe = ListSubscriber::model()->findAll($criteria);
			...
			$subscriber_id = implode(",",$subscriber_id_array);
			CommonHelper::setActivityLogs('SubscriberActive  '.str_replace(array('{{','}}'),'',$this->tableName()),$subscriber_id,$this->tableName(),'SubscriberActive',$customer->customer_id);
    ...
    public function getStatusesList()
    {
        return array(
			...
			self::STATUS_STOP   => Yii::t('list_subscribers', ucfirst(self::STATUS_STOP)),
        );
    }
