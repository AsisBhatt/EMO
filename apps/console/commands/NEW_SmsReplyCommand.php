				...
				if($receive->id != ''){
					...
					if(!is_array($receive_sms_array)){
						if($attributes_array['direction'] == 'inbound'){
							...
							$reply_customer_array = Yii::app()->db->createCommand("SELECT * FROM uic_beelift_number WHERE number = '".$attributes_array['to']."'")->queryAll();
							
							foreach($reply_customer_array as $reply_customer)
							{
								if(strstr($attributes_array['body'],'STOP')){
									
									$stop_number_array = Yii::app()->db->createCommand("SELECT lival.subscriber_id 'uic_list_field_value', lisu.list_id 'uic_list_subscriber' FROM uic_list_field_value lival, uic_list li, uic_list_subscriber lisu WHERE lival.subscriber_id = lisu.subscriber_id  AND li.list_id = lisu.list_id AND li.customer_id = '".$reply_customer['customer_id']."' AND lival.value = '".$attributes_array['from']."'")->queryAll();
								...
								$smsrply_model->customer_id = $reply_customer['customer_id'];
