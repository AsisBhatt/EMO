...
class SmsScheduleCommand extends ConsoleCommand 
{
    public function actionIndex() 
    {
		...
		$criteria->select = 't.sms_campaign_id,t.customer_id,t.campaign_name,t.send_at,t.list_id,t.campaign_text,campaign_status';
		...
		if(is_array($sms_campaigns) && count($sms_campaigns)){
			foreach($sms_campaigns as $smscampaign){
				...
				if($customer->status == 'active'){
							...
							if($rec['value'] != ''){							
								if($smscampaign->campaign_status == 'PROCESSING'){
									if(!$customer->getIsSmsOverQuota()){
										$status = $sms->shceduleSms($rec['value'], $newmessage, $smscampaign->customer_id);
										$status = explode("_",$status);
										if($status[0] == 'sent'){
											CommonHelper::setActivityLogs('SMS Campaign '.str_replace(array('{{','}}'),'',$sms->tableName()),$status[1],$sms->tableName(),'SMS Campaign',$smscampaign->customer_id);
											$count_rec++;
										}else{
											CommonHelper::setActivityLogs('SMS Campaign '.$status[0].' '.str_replace(array('{{','}}'),'',$sms->tableName()),$status[1],$sms->tableName(),'SMS Campaign',$smscampaign->customer_id);
											$count_rec_not_sent++;
										}
									}else{
										CommonHelper::setActivityLogs('SMS Campaign Merchant Validity is Over'.str_replace(array('{{','}}'),'',$sms->tableName()),0,$sms->tableName(),'SMS Campaign',$smscampaign->customer_id);
										$count_rec_not_sent++;
									}
								}else {
									CommonHelper::setActivityLogs('SMS Campaign Status is Stop'.str_replace(array('{{','}}'),'',$sms->tableName()),0,$sms->tableName(),'SMS Campaign',$smscampaign->customer_id);
									$count_rec_not_sent++;
								}	
							}else{
								CommonHelper::setActivityLogs('SMS Campaign Mobile Number is Not available '.str_replace(array('{{','}}'),'',$sms->tableName()),0,$sms->tableName(),'SMS Campaign',$smscampaign->customer_id);
								$count_rec_not_sent++;
							}
						}
					}
					...
					$adminemailBody = '<h2 style="border-collapse: collapse; font-family: Arial,sans-serif; font-size:20px;">SMS Campaign Report</h2>
					<table align="left" cellpadding="2" cellspacing="0" width="100%" bgcolor="#ffffff" style="border-collapse: collapse; font-family: Arial,sans-serif; font-size:14px; text-align:left;border:1px solid #666666;" >
					<tr>
					...
					<tr>
						...
						<td style="border:1px solid #666666; padding:8px;"> USD '.($count_rec * 0.0075).'</td>
					</tr>
					</table>';
					...
					$smscampaign->saveSendrecord($count_rec,$count_rec_not_sent,$smscampaign->sms_campaign_id);//$smscampaignquota->updateSendrecord($smscampaign->sms_campaign_id,$count_rec,$count_rec_not_sent);
					
					//} else{
						// $sms_campaign_error_log['error']['status_inactive'] = 'Customer Status is Inactive.';
					// }
				}else{
					CommonHelper::setActivityLogs('SMS Campaign Merchant is Not Active '.str_replace(array('{{','}}'),'',$sms->tableName()),0,$sms->tableName(),'SMS Campaign',$smscampaign->customer_id);
				}
			}
		}
    }
